...
use crate::config::{
    NDI_TRADEMARK, PLUGIN_AUTHOR, PLUGIN_GROUPING, PLUGIN_IDENTIFIER, PLUGIN_LABEL,
};
...
const _: &str = PLUGIN_AUTHOR;
const _: &str = NDI_TRADEMARK;
...
struct NetworkVideoFilter;

impl ImageEffectPlugin for NetworkVideoFilter {
    const IDENTIFIER: &'static CStr = c"jp.mikanseilaboratory.OpenFXNetworkVideo";
...
fn action_describe(effect: OfxImageEffectHandle) -> OfxResult<()> {
    ...
    props.set_string(kOfxPropLabel, 0, c"Network Video Output")?;
...
fn action_render(effect: OfxImageEffectHandle, in_args: OfxPropertySetHandle) -> OfxResult<()> {
    ...
    let instance = get_instance_data::<PluginInstance>(suites, effect)?;
    let converted = pixels::pass_rgba(
        &source,
        &output,
        window,
...
export_image_effect_plugin!(NetworkVideoFilter);
...
mod tests {
    ...
    fn identifier_matches_config() {
        assert_eq!(
            NetworkVideoFilter::IDENTIFIER.to_str().unwrap(),
            PLUGIN_IDENTIFIER
        );
    }
}
