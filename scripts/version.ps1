...
$cargoToml = Join-Path $PSScriptRoot "../crates/openfx-network-video-plugin/Cargo.toml"

function Get-CargoVersion {
    $match = Select-String -Path $cargoToml -Pattern '^version = "(.+)"'
    if (-not $match) {
        throw "version not found in crates/openfx-network-video-plugin/Cargo.toml"
    }
